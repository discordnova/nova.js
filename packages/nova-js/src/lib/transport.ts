...
export class Transport {
  // Nats connection
  public nats: NatsConnection | null = null;
  ...
  private async _subscriptionTask(topic: string) {
    ...
    for await (const publish of subscription) {
      try {
        ...
        const camelCasedName: CamelCase<typeof event.t> = event.t
          .toLowerCase()
        ...
        if (
          camelCasedName === 'interactionCreate' &&
          event.t === 'INTERACTION_CREATE'
        ) {
          ...
          this.emitter.emit<CamelCase<typeof event.t>>(
            camelCasedName as CamelCase<typeof event.t>,
            {
              ...event.d,
              client: this.emitter,
